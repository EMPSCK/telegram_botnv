...
from keyboards import scrutineer_list_active_comp_kb
...
async def cmd_start(call: types.CallbackQuery):
    ...
    if user_status == 2:
        ...
        await call.message.edit_text(text=text, reply_markup=scrutineer_start_kb.menu_kb)
...
async def cmd_start(call: types.CallbackQuery):
    user_status = await get_user_status_query.get_user_status(call.from_user.id)
    if user_status == 2:
        markup = scrutineer_list_active_comp_kb.gen_list_comp(call.from_user.id)
